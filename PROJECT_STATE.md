...
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.34`, assembly/internal root `VieriNexus`.
...
- `GearShopping.cs` and `Services/GearShoppingRuntimeService.cs` — Nexus-owned raw equipment/catalog contract, job-aware per-slot candidate ranking, manual upgrade preview/approval models, and full-run coordinator. Exact selected slots, item IDs, quantities, maximum unit prices, equipment signature, character, and gil floor form one single-use approval; Teleport/Navigation/Movement/UI/Inventory leases remain held through completion or confirmed Stop.
...
The 0.1.0.34 source passes all 191 tests plus a zero-warning full plugin build.
...
The user accepted the 0.1.0.33 manual preview with a valid live no-upgrade result: `Found 0 verified upgrade option(s) for MCH.` No movement or spending began. Version 0.1.0.34 changes the provider boundary so VieriAutoDuty 1.0.0.442 returns raw current equipment and ordinary gil-vendor catalog facts rather than the already-selected per-slot winners. Nexus now applies job-primary-stat scoring, best-candidate ordering, weakest-ring comparison, active EXP-item protection, and effective two-handed/off-hand rejection before it renders or approves a replacement. The exact single-use approval and provider-side fresh-plan recheck remain defense in depth. Vendor-band/catalog discovery and low-level purchase/equip/gearset/displaced-item mechanics are still temporary adapter responsibilities.
...
- **Remaining AutoDuty migration scope:** route composition, bounded duty execution, automatic gear readiness, and the 0.1.0.33 manual preview are live-accepted. Version 0.1.0.34 gives Nexus role-aware per-slot candidate ranking in addition to the manual UI, exact approval, spending policy, resource lifecycle, and Stop. VieriAutoDuty provides raw catalog/equipment facts plus temporary vendor/equip mechanics. Vendor-band/catalog discovery, low-level purchase/equip/gearset/displaced-item mechanics, maintenance, configuration import, the rest of the preserved custom overlay/status/control experience, and final stock-provider coexistence/retirement remain governed by `docs/AUTODUTY_PROVIDER_MIGRATION_AUDIT.md`.
