...
Working snapshot: 2026-09-22 (America/New_York)
...
The 2026-09-22 maintenance baseline is VieriCodex `1.12.2.85` / source `4f27a3db9` over Questionable `15.756.3.9`; VieriAutoDuty `1.0.0.449` / source `f73ddf11` over AutoDuty `0.0.0.346`; and VieriRotationHelper `2.0.0.12` / source `6cbd001` over Wrath Combo `1.0.4.25` plus WrathCombo.API `0.5.8`. VieriAutoDuty `.449` skips unreadable territory files during optional repair/shop NPC discovery, preserving the remaining vendor scan and allowing settings, commands, and IPC to register. Fatal startup failures now surface as real Dalamud load errors instead of an apparently loaded, nonfunctional plugin. VieriCodex `.85` keeps its optional damage-interrupt hook from aborting startup when Dalamud's default Reloaded backend cannot reserve a nearby memory buffer. The `.84`/`.447` hotfix repairs the Progression Queue handoff after AutoDuty's profile-v2 migration. VieriAutoDuty `.448` restores every custom control to its original category and recognizes VieriAvarice as the positional replacement. Daily Pilcrow release `bb998f83c91602a375d60e32500984cf24ce9fdc` is live; focused HTTP/ZIP/hash verification and Discord workflow `35688482646` succeeded. The live feed still retires `MarkerIconPriority`, `WrathSwitch`, `VieriHildaLayer`, and `VieriNexus`; their local repositories remain untouched. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
