...
Current product version: `0.1.0.79`
Current published source: `0.1.0.79`, source `6bc0e58befe1379776d82e0353f3c2e47898cd65`, website release `98ed75c5c47f8f69c3aa7ba8dfcebbf2febf9ddf`, verification documentation `9cb796389e7aa3a0b1c9831f02c95415a78100d6`. Release deployment `dpl_G5H2tVYgRzs6iLvK7eyjzVRjDgs3` and final documentation deployment `dpl_BqdaXi9HjCybzwDTAmrBgAJAnuxk` are Ready; focused live verification passed for runtime/source ZIP validity and exact SHA-256 `253D70239264103AB3A4B8B5FF0EE4E636603DBAF11240EC1C6BE6F585FA98F3` / `EBB9E9C25937D0E0D75B56F97070A2D252ACF0EE66B6A1DC7D8D5279277A562A`; Discord workflow `35485984942` succeeded. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.79 is published and restores the complete VieriCodex One Click Navigation surface inside Custom Route Editor. Nexus can arm the next FFXIV map click, use an existing flag, capture an optional shortcut, report destination/status, and stop the owned trip. Same-zone and cross-zone destinations run through the normal Nexus coordinator over stock Lifestream and vnavmesh, and two-dimensional map coordinates are projected onto the destination mesh only after the target territory loads. The compact operations overlay's Flag Marker uses the same service instead of a separate current-zone-only implementation. The overall consolidation remains active and must continue against `VieriFeatureParity` and the predecessor behavior list rather than waiting for user-discovered omissions.
...
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.79`, assembly/internal root `VieriNexus`; builds and packages the five isolated custom runtime archives.
...
- Plugin project version: `0.1.0.79`. Dalamud API 15. Version 0.1.0.79 is the current production feed entry; every prior version remains an immutable historical archive.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
