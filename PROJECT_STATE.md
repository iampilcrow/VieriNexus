...
The 2026-09-20 maintenance baseline is VieriCodex `1.12.2.84` / source `678789e6e` over Questionable `15.756.3.9`; VieriAutoDuty `1.0.0.446` / source `8c59d633` over AutoDuty `0.0.0.346`; and VieriRotationHelper `2.0.0.12` / source `6cbd001` over Wrath Combo `1.0.4.25` plus WrathCombo.API `0.5.8`. The `.84`/`.446` hotfix repairs the VieriCodex Progression Queue handoff after AutoDuty's profile-v2 migration by using canonical reversible `Meta` settings paths and the current object-valued settings IPC signature. Daily Pilcrow release `40545cff9f94d8974dcb1a4b34d5b56296da5648` is live through Vercel deployment `9wRGacFU3EapPZicwCHnMxqUFok1`; focused HTTP/ZIP/hash verification and Discord workflow `35529620683` succeeded. The live feed also retires `MarkerIconPriority` (VieriMarkerIcon), `WrathSwitch` (VieriWrathSwitch), and `VieriHildaLayer`; their local repositories remain untouched. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
