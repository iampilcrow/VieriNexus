...
Current product version: `0.1.0.63`
...
Current workstream: version 0.1.0.63 completes the ordinary duty-provider cutover. Stock AutoDuty is the only executable duty candidate; VieriAutoDuty remains detectable solely as a settings-migration source and is rejected if loaded. A verified ordinary duty or duty-only Grand Company Hunting Log target now flows automatically through the enabled Nexus between-duty maintenance profile, a distinct completion checkpoint, gear readiness, and then the next bounded activity. Last Run pauses before maintenance, and stopped or failed maintenance cannot advance the goal. Version 0.1.0.62 remains the overlay/provider-identity hardening foundation and 0.1.0.61 remains the embedded-runtime consolidation foundation.
