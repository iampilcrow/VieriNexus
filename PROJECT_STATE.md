...
Current product version: `0.1.0.35`
Current published release source: `0.1.0.35`, commit `8dec5406291d44bed621b1f0de5821773e004bde`. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
...
The current production line is an active migration foundation. Nexus can be installed alongside existing Vieri products, transactionally imports VieriNavPlotter into immutable staging, maintains a separate Nexus-owned working library, records and edits personal routes, previews saved/generated paths, and performs guarded local, cross-zone, or Grand Company inn travel. Version 0.1.0.26 added the first Nexus-owned Progression planner. Versions 0.1.0.27-0.1.0.29 removed the route dependency on VieriAutoDuty and completed accepted Nexus-owned Lifestream/vnavmesh travel. Versions 0.1.0.30-0.1.0.31 added and live-accepted the first durable one-duty-at-a-time Progression execution lane. Version 0.1.0.32 added and live-proved the separate Nexus-owned gear-readiness transaction before duty selection. Version 0.1.0.33 added and live-accepted the first Gear & Inventory page, and 0.1.0.34 moved role-aware per-slot ranking into Nexus. Version 0.1.0.35 reads live/owned equipment and ordinary gil-vendor catalogs directly, owns vendor-band/job-family discovery, and uses the same exact Nexus approval for manual and automatic Progression gear work. VieriAutoDuty is temporarily retained only for physical shop/purchase/equip/gearset/displaced-item actions. Quest execution and those last equipment mechanics remain locked. VieriCodex and VieriAutoDuty are migration providers, not permanent selections.
...
- `ProgressionExecution.cs` — character-scoped atomic Reach Job Level desired state and task history; separate gear-readiness and one-duty task kinds; exclusive resource acquisition; hard gil-floor and non-regression verification; provider-setting restoration/reconciliation; exact duty completion; Last Run; Stop; and no-replay reload recovery.
...
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.35`, assembly/internal root `VieriNexus`.
...
- `Services/ProgressionProviderService.cs` — VieriCodex, stock Questionable, VieriAutoDuty, and stock AutoDuty presence plus IPC-contract probe. Questing remains observation-only. The duty edge exposes one exact bounded `Run`, status/path observations, and `Stop`; it resets provider leveling mode through either direct `SetLevelingMode` or stock `SetConfig`, distinguishes stock/transition flavor, and rejects simultaneous ready quest providers. Its temporary Vieri-only gear adapter applies/restores an ephemeral smart-buy/gil-floor override, starts/stops bounded gear readiness, reads telemetry, invalidates duty eligibility, publishes live item-level/gil metrics, and bridges versioned read-only upgrade scans plus exact approved-shopping requests.
- `GearShopping.cs`, `Services/NexusGearCatalogService.cs`, and `Services/GearShoppingRuntimeService.cs` — Nexus-owned live equipment/owned-item scan, curated vendor-band/job-family selection, ordinary gil-shop catalog traversal, job-aware per-slot candidate ranking, automatic/manual exact approval, and full-run coordinator. Exact selected slots, item IDs, quantities, maximum unit prices, equipment signature, character, and gil floor form one single-use approval; Teleport/Navigation/Movement/UI/Inventory leases remain held through completion or confirmed Stop.
...
The 0.1.0.35 source passes all 194 tests plus a zero-warning full plugin build.
...
`ReachJobLevelPlanner` is the first concrete desired-state planner. It validates current character/job/level, target level, a hard non-negative gil reserve, at least one allowed leveling method, and compatible provider availability. It can propose Nexus-owned gear readiness, supported quest work, exactly one bounded duty, and post-activity level verification/replanning. An unavailable optional lane is a warning when another allowed lane is ready; no ready provider or an ambiguous double-provider state blocks the draft. Gear readiness and the bounded duty lane are connected to execution; quest work remains planning-only.

The current character's draft persists through configuration schema 3. `ProgressionExecutionCoordinator` and `FileProgressionGoalStore` durably own one character-scoped Reach Job Level desired state, plan revision, bounded task history, active task, Last Run flag, provider-start checkpoint, duty-entry checkpoint, and matching game duty-completion checkpoint. Before selecting a duty, Nexus creates a separate gear-readiness task, owns its travel/UI/inventory resources, enforces/restores the gil policy, and verifies live item level/gil. Nexus then selects one eligible unlocked duty, atomically acquires its full resource bundle, disables the provider's internal leveling scheduler, calls only stock-compatible bounded AutoDuty IPC, verifies completion and return to the world, then replans. Stop, Last Run, provider loss, job change, reload, and unload are reconciled without replaying stale work. There is still no general cross-module goal repository/task graph executor; quest execution and detailed Nexus-native shopping/equipment policy remain unfinished.
...
| --- | --- |
| VieriAutoDuty | `0a81501e7f00d682ad66211080fb2a2f1ae92fe0` |
...
Home, Control Center, Progression, Routes, Dependencies, Migration, and Settings have specialized current implementations. Other destinations render an honest staged-module placeholder explaining that no live behavior has moved.
...
`Configuration` implements Dalamud `IPluginConfiguration`, currently schema `Version = 3`:
...
`Initialize(...)` clamps UI scale, restores dictionary comparers/null safety, supplies missing per-character Progression drafts, sets schema version 3, and attaches the plugin interface. There is no destructive older-version migration; existing character and route settings remain intact.
...
- Plugin project version: `0.1.0.35`. Dalamud API 15. The release is published and publicly verified.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
...
**Completed foundation:** solution layering, shell/Home/dependency/setup UI, basic character/world readiness, neutral module descriptors, domain contracts, tested lease/verified-Stop/reload-watchdog/automatic-authority/recovery primitives, explicit route-Stop policy, live navigation provider health and bounded transition audit, historical isolated safety simulation, read-only status/dependency/navigation/activation/override-resolution IPC, nine-source read-only discovery, exact source lock, first transactional importer, immutable verified staging, a separate atomic working-library store, provider-neutral route planning, manual and timed route authoring, detailed point editing and safe route exchange, the immutable 27-route vendor catalog, exact-target one-winner assignment and current-target capture, static and ownership-filtered generated-path preview, Nexus-owned same/cross-zone/vendor/inn route composition over Lifestream and vnavmesh, immediate button-only Stop/restart, a source-level VieriAutoDuty-to-stock provider migration audit, character-scoped Progression draft policy, stock/transition quest and duty contract probes, conflict rejection, bounded Reach Job Level plan preview, the durable one-duty-at-a-time lifecycle with exact completion/Last Run/Stop/reload reconciliation/replanning, Nexus-owned live equipment/owned-item and ordinary gil-shop catalog discovery, role-aware automatic/manual exact upgrade approvals, protected gil policy, full resource ownership, temporary mechanics configuration, setting restoration, and postcondition verification.

**Partially complete:** world state, dependency health, module contract, IPC surface, configuration migration framework, ownership, solo-duty policy, navigation migration, and Progression execution. Progression duties and the first gear-readiness transaction are live; quest execution, exact Nexus-native shopping/slot/equip policy, maintenance, and the preserved custom overlay are not.

**Not implemented:** automatic replay of interrupted movement (intentionally forbidden); durable general audit/history beyond the character-scoped Progression task document; other eight transactional importers; Communications secret adapter; general multi-module scheduler/executor/reconciler; command gateway/event bus; SQLite history; diagnostics/support export; legacy IPC aliases; migrated quest/gear/combat/market/communications/command-center/custom-UI runtimes; standalone retirement.
...
- **Remaining AutoDuty migration scope:** route composition, bounded duty execution, automatic gear readiness, and manual Gear & Inventory are live-accepted. Version 0.1.0.35 gives Nexus the live equipment/owned-item scan, vendor-band/job-family/catalog discovery, ranking, automatic/manual exact approval, spending policy, resource lifecycle, and Stop. VieriAutoDuty provides only temporary physical shop/purchase/equip/gearset/displaced-item mechanics for this lane. Those actions, maintenance, configuration import, the rest of the preserved custom overlay/status/control experience, and final stock-provider coexistence/retirement remain governed by `docs/AUTODUTY_PROVIDER_MIGRATION_AUDIT.md`.
...
4. Treat the repeated 0.1.0.34 zero-upgrade MCH result as accepted. Confirm one 0.1.0.35 refresh produces the same result from Nexus's native equipment/catalog reader; no purchase or unrelated regression test is required.
5. Move the remaining physical shop/purchase/equip/gearset/displaced-item actions into Nexus as one implementation slice, preserving vendor-route and exact-approval safeguards. The current Vieri adapter is temporary mechanics, not permanent policy ownership.
...
8. For every substantial change, update this file, `IMPLEMENTATION_STATUS.md`, migration/upstream policy, source lock, tests, package metadata, and release documentation consistently.
...
- **Current production release:** `0.1.0.35`.
...
Verification evidence for 0.1.0.35: Nexus source `8dec5406291d44bed621b1f0de5821773e004bde` and Daily Pilcrow release `e2a1f5ff7d420a9a341c112585469748a889274d` are pushed. All 194 Nexus tests, the zero-warning clean Nexus Release build, all 205 website tests, typecheck, focused package validation, thirteen-entry inventory guard, and production website build pass. Production deployment `dpl_FBMRgFqf9dgvcLLW23XWgPnpKmVQ` is Ready. The live feed advertises VieriNexus 0.1.0.35; its public runtime/source downloads return HTTP 200 as valid ZIPs and match SHA-256 `1DDED65D3CD5D489F1417CCD3613E349BA4915A1CDB328EA9848F435C9DEBA5E` / `7FCF06A97210227A7D42461350311999F8B876249B05951B9E66EDF9FDD4CD1E`. GitHub Actions Discord workflow `34554855142` completed successfully. Manual and automatic gear planning now read equipment and supported vendor catalogs directly in Nexus; the focused user gate is one MCH refresh returning the already-accepted zero result. No purchase, Routes, duty, or Progression run is required.
