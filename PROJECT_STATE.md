...
Current product version: `0.1.0.58`
Current published source: `0.1.0.58`, source `41b1f9c829cc995176708603fc536a572f3a9ff6`, website release `3841aa3b4619458097bb78f9bb42e2a06ee3db56`, verification documentation `249fc004c50fe30b11ec4f2c1c054da03c17ecf2`. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.58 is published and closes the five-route stock-Questionable compatibility boundary without any upstream submission. Nexus validates exact quest/sequence/step identities and expected old values, creates a hash-addressed byte-identical backup of Questionable's complete official bundle, replaces only the required entries atomically, and reloads stock Questionable only after its public activity contract confirms it is idle. Any missing, duplicate, reshaped, concurrently replaced, or unexpectedly changed route aborts the whole write. A new Questionable bundle revokes readiness and is rechecked automatically; an affected quest cannot start until the corrected bundle is actually loaded, while every unrelated quest and all execution remain stock-owned. Native upstream corrections are accepted without a write, and guarded rollback restores the exact official bundle. The user explicitly directed that the isolated branch is not to be pushed or submitted upstream. The next broad slice is native multi-job queue execution and final VieriCodex retirement. Separately, stock AutoDuty 0.0.0.335's only remaining generic recovery difference is isolated with five passing policy tests on branch `nexus-recovery-fixes-0.0.0.335` at `2c583870c216573a054af109d11fafdd85d556be`.
...
The 0.1.0.58 release candidate passes all 313 tests plus a zero-warning full plugin build.
...
- Plugin project version: `0.1.0.58`. Dalamud API 15. Version 0.1.0.57 remains published through the production feed until the 0.1.0.58 release completes.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
...
2. **NEXT SUBSTANTIAL SLICE — native multi-job queue and stock quest retirement:** execute the complete migrated queue definition natively across job switches, preserve Nexus-owned activity/Atlas policy, and remove VieriCodex from quest-provider selection after a single representative stock acceptance pass. The five isolated route corrections are now closed by Nexus's guarded stock-bundle compatibility layer; no upstream submission is planned.
