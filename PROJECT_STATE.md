...
- **Progress Atlas** — Nexus-owned per-character completion for Aetherytes/Aethernet, all open-world and quest-earned Aether Currents, and current non-Legacy achievements. It reads current game data directly and does not call VieriCodex; Hunting/Grand Company Logs and world exploration are the next categories to connect to the same model.
- **Progress Atlas next source slice (unreleased)** — adds Mapping/Remapping world regions plus exact current class/Grand Company target kill progress and current-rank target details to that same Nexus-owned model. The selector is implemented and tested; execution remains deliberately unavailable until stock movement/combat boundaries are wired safely.
