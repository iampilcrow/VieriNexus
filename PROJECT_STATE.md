...
Current product version: `0.1.0.77`
Current published source: `0.1.0.77`, source `6d09526414a4614166f36385e633c883b8bf400a`, website release `008f9ce844d0ff0aac3cdb4388627bc7bba4555b`, verification documentation `31dfe6e74a0868e77027d67ea69f6b70582967c7`. Release deployment `dpl_DJWmac5MS4V3XKLesnQPFtAVfhW3` and final documentation deployment `dpl_GqHz2nU2LatCGn5inm9GomBoyCvz` are Ready; focused live verification passed for runtime/source ZIP validity and exact SHA-256 `E2A12B8D43F5960436889F6C3D8885F445E1FBB73C53BCC6DEAF8009E3FB100B` / `2004873CD9BA9BBFCFAB720581DB59B555145E394C24B7229AAB76BF0246ABC9`; Discord workflow `35484278454` succeeded. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.77 is published and makes predecessor behavior an explicit compatibility contract. Complete Market Helper, Custom UI, Communications, Rotation, and Positional settings are accessible inside Nexus; the exact VieriAutoDuty compact action surface is regression-locked; Automation opens stock Questionable directly while Nexus owns goals, safety, state, and Vieri-specific presentation. `VieriFeatureParity` records every VieriCodex activity lane and the intended stock-provider boundary so future cleanup cannot silently strip a predecessor feature. The overall replacement is not declared complete: the next large production slice is richer Progress Atlas parity, especially expansion/category drill-down and detailed quest, duty, and unlock inspection, while Questionable remains the ordinary quest executor.
...
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.77`, assembly/internal root `VieriNexus`; builds and packages the five isolated custom runtime archives.
...
- Branch: `main`.
- Current release source/implementation commit: `6d09526414a4614166f36385e633c883b8bf400a Complete embedded settings parity in Nexus`; publication is recorded in this current `PROJECT_STATE.md` update.
...
- Plugin project version: `0.1.0.77`. Dalamud API 15. Version 0.1.0.77 is the current production feed entry; every prior version remains an immutable historical archive.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
