...
Current published source: `0.1.0.57`, release source `a6b9cb9f32d5ac4404d16d6a5d0703cd1313ade0`, website release `e1d1da27887c6d976dd92d156ca361f75e54d0fa`. Version `0.1.0.58` is the active release candidate. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.58 closes the five-route stock-Questionable compatibility boundary without any upstream submission. Nexus validates exact quest/sequence/step identities and expected old values, creates a hash-addressed byte-identical backup of Questionable's complete official bundle, replaces only the required entries atomically, and reloads stock Questionable only after its public activity contract confirms it is idle. Any missing, duplicate, reshaped, or unexpectedly changed route aborts the whole write. A new Questionable bundle revokes readiness and is rechecked automatically; an affected quest cannot start until the corrected bundle is actually loaded, while every unrelated quest and all execution remain stock-owned. Native upstream corrections are accepted without a write, and guarded rollback restores the exact official bundle. The user explicitly directed that the isolated branch is not to be pushed or submitted upstream. The next broad slice remains native multi-job queue execution and final VieriCodex retirement. Separately, stock AutoDuty 0.0.0.335's only remaining generic recovery difference is isolated with five passing policy tests on branch `nexus-recovery-fixes-0.0.0.335` at `2c583870c216573a054af109d11fafdd85d556be`.
