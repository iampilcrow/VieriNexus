...
Current product version: `0.1.0.57`
Current published source: `0.1.0.57`, release source `a6b9cb9f32d5ac4404d16d6a5d0703cd1313ade0`, website release `e1d1da27887c6d976dd92d156ca361f75e54d0fa`. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.57 is published and live. The user has disabled VieriDeck, completing the first predecessor retirement. This slice adds Nexus-owned exact Main Scenario selection and durable execution, and a transactional VieriCodex settings handoff that preserves all activity/Atlas preferences, the level stop, every queue step, and queue behavior. A matching current-job queue step is converted into fresh Nexus desired state; no predecessor runtime pointer is replayed, and rollback restores the prior character settings. The next broad slice is native multi-job queue execution plus the final stock-Questionable retirement boundary. Stock Questionable 15.756.2.5 exposes the complete permanent one-quest contract. Its latest upstream `new-main` at `5a751c819` still lacks the five isolated route corrections; repository policy requires explicit human approval before rebasing/pushing/opening the upstream PR. Separately, stock AutoDuty 0.0.0.335's only remaining generic recovery difference is isolated with five passing policy tests on branch `nexus-recovery-fixes-0.0.0.335` at `2c583870c216573a054af109d11fafdd85d556be`.
...
The current production line is an active migration foundation. Nexus can be installed alongside existing Vieri products, transactionally imports VieriNavPlotter into immutable staging, maintains a separate Nexus-owned working library, records and edits personal routes, previews saved/generated paths, and performs guarded local, cross-zone, or Grand Company inn travel. Version 0.1.0.26 added the first Nexus-owned Progression planner. Versions 0.1.0.27-0.1.0.29 removed the route dependency on VieriAutoDuty and completed accepted Nexus-owned Lifestream/vnavmesh travel. Versions 0.1.0.30-0.1.0.31 added and live-accepted the durable one-duty-at-a-time Progression lane. Versions 0.1.0.32-0.1.0.35 progressively moved gear readiness, approval, ranking, live equipment, and vendor catalogs into Nexus. Version 0.1.0.36 completes native Gear & Inventory execution. Version 0.1.0.37 transactionally preserves the complete VieriAutoDuty operations/profile state and adds the compact Nexus-owned overlay. Version 0.1.0.38 promotes each user's verified operations import into an independent atomic working library, runs the safe native maintenance subset without AutoDuty IPC, restores the striking-dummy catalog/travel under Nexus ownership, and adds a one-action install-first migration path for each computer. Version 0.1.0.39 moves Class/Job/Role quest-family selection and the bounded quest lifecycle into Nexus. Version 0.1.0.40 extends that same exact-quest lifecycle to ordinary non-repeatable general side quests selected by Nexus from current game data. Version 0.1.0.42 adds the first native unified Progress Atlas categories—Aetherytes/Aethernet, all Aether Currents, and achievements—read directly from current per-character game state without VieriCodex, and replaces player-facing numeric job IDs with localized full names and abbreviations such as `Machinist (MCH)`. Version 0.1.0.43 adds live Mapping/Remapping exploration completion and all 12 Hunting/Grand Company Logs with 666 exact targets and current-character kill progress. Version 0.1.0.44 adds bounded open-world Hunting Log execution using stock providers only for narrow travel/path/rotation mechanics. Version 0.1.0.45 adds exact one-click travel-node, field-current, and exploration actions. Version 0.1.0.46 connects quest-earned currents and duty-only Grand Company targets through exact bounded provider work. Version 0.1.0.47 moves protected between-duty selling/desynthesis/turn-in/storage orchestration into Nexus. Version 0.1.0.48 completes Nexus-owned travel, approach, interaction, and verification prerequisites for Grand Company turn-ins and collection storage. Version 0.1.0.49 moves the VieriAutoDuty-specific command/status role and useful duty controls into a single Nexus gateway and the Nexus-owned Goto/Gear/Inventory/Duty/Extras overlay. Version 0.1.0.50 makes the shared AutoDuty identity fail closed on duplicate loaded implementations, identifies stock automatically after the fork is unloaded, synchronizes the live fork with current upstream, and reduces the final duty-engine audit to one generic recovery family. Version 0.1.0.51 applies the same fail-closed/automatic-stock handoff to VieriCodex and Questionable and records the final-tree migration audit. Version 0.1.0.52 moves the stock-Questionable solo-duty rotation/targeting lease into Nexus and reduces remaining route parity to five isolated corrections. Version 0.1.0.53 completes the transactional VieriDeck settings handoff, and 0.1.0.55 presents it correctly as the favorites-first Plugins page with inline commands and top-level Dalamud controls. Version 0.1.0.57 adds Nexus-owned exact MSQ selection/execution and the full transactional VieriCodex activity/Atlas/queue settings handoff. VieriDeck is now retired. VieriAutoDuty is no longer used for Routes, Gear & Inventory, maintenance, turn-ins, storage, striking-dummy travel, command routing, or suite telemetry; it remains the temporary bounded duty provider while in-duty leave/resume, the isolated recovery parity, and final stock-provider replacement migrate. VieriCodex and VieriAutoDuty are migration providers, not permanent selections.
...
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.53`, assembly/internal root `VieriNexus`.
...
- `Services/ProgressionProviderService.cs` — VieriCodex, stock Questionable, and every installed implementation sharing the AutoDuty identity plus narrow IPC-contract probes. Nexus enumerates and classifies current game-data Class/Job/Role and ordinary general side quests, applies current-job and provider eligibility, dispatches one exact quest, observes its exact ID/completion, session-blacklists pathless candidates, and exposes bounded Stop. It blocks VieriCodex and stock Questionable when both are loaded before invoking either contract, then automatically selects stock after the fork is disabled. The duty edge exposes one exact bounded `Run`, status/path observations, and `Stop`; it resets provider leveling mode through either direct `SetLevelingMode` or stock `SetConfig`, automatically selects the sole loaded stock provider after VieriAutoDuty is unloaded, and blocks dispatch before IPC use if duplicate `AutoDuty` identities are loaded. Gear planning and execution delegate only to Nexus-owned services; no AutoDuty gear IPC remains.
...
There are 304 automated tests across the application/domain policies, including MSQ planning/execution priority, null-safe complete VieriCodex settings/queue import, transactional VieriCodex recovery/rollback, `NexusCommandPolicyTests.cs` for aliases, public-version alignment, payload bounds, character scoping, and rejected unsafe controls, provider identity handoff coverage, Command Center importer/atomic-working-copy coverage, and `PluginPagePolicyTests.cs` for favorites-first grouping and exact inline command-panel toggling.
...
The published 0.1.0.57 release passes all 304 tests plus a zero-warning full plugin build.
...
- `/vierinexus`
- `/nexus`
- Page subcommands: `home`, `splash` (alias of Home), `show`, `hide`, `dependencies`, `migration`, `routes`, `progression`, `atlas`, and `commands`/`commandcenter`/`deck` aliases.
...
`Configuration` implements Dalamud `IPluginConfiguration`, currently schema `Version = 4`:
...
- Plugin project version: `0.1.0.57`. Dalamud API 15. Version 0.1.0.56 remains published through the production feed until the 0.1.0.57 release completes.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
...
Version 0.1.0.57 is published from Nexus source `a6b9cb9f32d5ac4404d16d6a5d0703cd1313ade0` through Daily Pilcrow release `e1d1da27887c6d976dd92d156ca361f75e54d0fa`. Nexus now owns exact Main Scenario quest selection, durable identity, completion verification, and replanning through the stock-compatible one-quest contract. The transactional VieriCodex handoff preserves all activity and Progress Atlas preferences, level stop, full saved queue definitions, and queue behavior, converts only matching current-job intent into fresh Nexus desired state, and supports guarded rollback. All 304 Nexus tests, the zero-warning Release build, all 205 website tests, typecheck, focused package validation, the 13-entry inventory guard, production build, and public HTTP/ZIP/hash checks pass. Production deployment `dpl_DaQpTJxRQBN5uDLAP9UvtsiSqdfH` is Ready and Discord workflow `34717702996` succeeded. Runtime/source SHA-256: `BBEDF2BCF95E90EA0A03685D4E684AC2BD068575195C0FE4918796AB623DD1FC` / `E72AF87B04FE0309D9133E82723CA17E850DC1DE8878005847CD0BA8B0C11D75`.
...
2. **NEXT SUBSTANTIAL SLICE — native multi-job queue and stock quest retirement:** execute the complete migrated queue definition natively across job switches, preserve Nexus-owned activity/Atlas policy, close the five isolated route corrections through the approved upstream path, and remove VieriCodex from quest-provider selection after a single representative stock acceptance pass.
...
10. Treat 0.1.0.51 as the equivalent Questionable identity handoff boundary and 0.1.0.52 as the Nexus-owned stock solo-duty rotation boundary. VieriCodex and stock Questionable may never both delegate work; stock is selected automatically after the fork is disabled; the five remaining route corrections and protected regressions live in `docs/QUESTIONABLE_PROVIDER_MIGRATION_AUDIT.md`. Future compatible Questionable updates should require a contract/content audit, not a Nexus code change or permanent Vieri fork merge.
