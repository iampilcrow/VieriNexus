...
Current product version: `0.1.0.59`
Current published source: `0.1.0.59`, source `7faf4cd5faad282a13e747f386c5195e91f44e67`, website release `6d938c60b11fdae6cc6eb8dc7e061eea3acff072`, verification documentation `9f6033e35281d5e2704940e242ceeaa9bc168917`. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.59 is the published native multi-job queue and final VieriCodex runtime-retirement release. Nexus owns every imported queue step, target, method, fallback, advancement setting, restart state, and completion checkpoint per character. It switches through Fast Job Switcher's proven lower-case slash-command contract, requires that plugin as a core dependency, waits for the game to confirm the exact requested job, settles the gear change, and then creates a fresh bounded Nexus goal. Existing verified 0.1.0.57/0.1.0.58 imports are promoted once automatically; new/friend computers still install Nexus and import locally before disabling predecessors. Stock Questionable is now the only quest runtime candidate; VieriCodex is detected only as a migration-source conflict and receives no runtime IPC calls. The next broad consolidation slice is the remaining AutoDuty provider/recovery retirement boundary. Separately, stock AutoDuty 0.0.0.335's remaining generic recovery difference is isolated with five passing policy tests on branch `nexus-recovery-fixes-0.0.0.335` at `2c583870c216573a054af109d11fafdd85d556be`.
...
- `Configuration.cs` — schema 8 global presentation/setup/operations-overlay settings, imported-operations preference receipt, character-scoped safety, Progression draft and complete multi-job queue state, one-time verified queue promotion, and per-source migration state.
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.59`, assembly/internal root `VieriNexus`.
...
There are 320 automated tests across the application/domain policies, including native queue import/normalization/order/method behavior, MSQ planning/execution priority, null-safe complete VieriCodex settings import, transactional recovery/rollback, command aliases, public-version alignment, payload bounds, character scoping, rejected unsafe controls, provider identity handoff coverage, and Plugins-page grouping/command behavior.
...
The 0.1.0.59 release candidate passes all 320 tests plus a zero-warning full plugin build.
...
- Plugin project version: `0.1.0.59`. Dalamud API 15. Version 0.1.0.58 remains published through the production feed until the 0.1.0.59 release completes.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
