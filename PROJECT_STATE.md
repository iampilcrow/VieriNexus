...
Working snapshot: 2026-09-21 (America/New_York)
...
The 2026-09-21 maintenance baseline is VieriCodex `1.12.2.85` / source `4f27a3db9` over Questionable `15.756.3.9`; VieriAutoDuty `1.0.0.448` / source `4e3a7014` over AutoDuty `0.0.0.346`; and VieriRotationHelper `2.0.0.12` / source `6cbd001` over Wrath Combo `1.0.4.25` plus WrathCombo.API `0.5.8`. VieriCodex `.85` keeps the optional damage-interrupt hook from aborting the complete plugin when Dalamud's default Reloaded backend cannot reserve a nearby memory buffer; the hook now uses MinHook while every Vieri feature and ordinary Questionable path remains unchanged. The `.84`/`.447` hotfix repairs the VieriCodex Progression Queue handoff after AutoDuty's profile-v2 migration by using canonical reversible `Meta` settings paths, the current object-valued settings IPC signature, and explicit preservation/restoration of AutoDuty's verified leveling duty across the profile setters' intentional selection reset. VieriAutoDuty `.448` removes the temporary top-level Vieri settings catch-all, restores every custom control to its original AutoDuty category, and recognizes VieriAvarice as the installed Avarice replacement while preserving the direct VieriAvarice-to-Boss-Mod positional bridge. Daily Pilcrow release `4cf107826b64f1499e10ad278205ccd8005a564b` is live; focused HTTP/ZIP/hash verification and Discord workflow `35559998793` succeeded. The live feed also retires `MarkerIconPriority` (VieriMarkerIcon), `WrathSwitch` (VieriWrathSwitch), and `VieriHildaLayer`; their local repositories remain untouched. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
