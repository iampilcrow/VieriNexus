...
Current workstream: version 0.1.0.47 is the protected between-duty item-transaction release. Nexus owns exact reviewed selling, protected native desynthesis, and bounded Grand Company/storage provider orchestration without AutoDuty maintenance IPC. Safe in-duty leave/resume remains blocked on a missing stock AutoDuty contract; the next broad slice continues the remaining predecessor modules rather than subdividing this boundary.
...
3. **AUTODUTY DECOUPLING:** Routes, Gear & Inventory, Progression scheduling/Last Run, protected between-duty maintenance, narrow turn-in/storage provider orchestration, striking-dummy travel, operations settings, and the compact overlay are now Nexus-owned. Finish safe in-duty leave/resume, telemetry/command parity, and the remaining duty-engine audit while keeping the stock duty engine external and updateable.
