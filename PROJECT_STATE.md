...
Current product version: `0.1.0.78`
Current published source: `0.1.0.78`, source `2d483f7ab8a5424ecc9b8d7caa8400b7aae85205`, website release `70a1a67412648f804eb80582529ed8c6be4bf1da`, verification documentation `c6c9941b42f56eee58ab883e2fb63581f05775c2`. Release deployment `dpl_E9WP7U4pmEHmthidcmSna7omM8sQ` and final documentation deployment `dpl_6ruZTksJVCiTTdqX5mMvVnKGkGZK` are Ready; focused live verification passed for runtime/source ZIP validity and exact SHA-256 `5FC43A1F8C6D8BBD3857422229B5E7BE8D027725F5AB7371120BADC46E9121A2` / `9C75CA71C0DBD223E095F8AE6EB920D44F8B0C3A622FA1548D5784AE332A22FF`; Discord workflow `35485097391` succeeded. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.78 is published and restores the VieriCodex-style expansion-first Progress Atlas. Each expansion now exposes MSQ, current-job Class/Job/Role quests, Side Quests, Aetherytes/Aethernet, field and quest Aether Currents, world exploration, duty exploration, and categorized duty unlocks. Nexus owns the unified catalog, search, status presentation, and bounded coordination; stock Questionable supplies live quest state and exact Start/Continue execution, while stock AutoDuty receives only duties whose public contract confirms a runnable path. Exact duty-unlock quests are shown and startable when current game data exposes them. Live quest reads are budgeted and cached to prevent the large catalog from freezing Dalamud's draw loop. The overall consolidation remains active and must continue against `VieriFeatureParity` and the predecessor behavior list rather than waiting for user-discovered omissions.
...
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.78`, assembly/internal root `VieriNexus`; builds and packages the five isolated custom runtime archives.
...
- Branch: `main`.
- Current release source/implementation commit: `2d483f7ab8a5424ecc9b8d7caa8400b7aae85205 Restore expansion progress atlas parity`; publication is recorded in this current `PROJECT_STATE.md` update.
...
- Plugin project version: `0.1.0.78`. Dalamud API 15. Version 0.1.0.78 is the current production feed entry; every prior version remains an immutable historical archive.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
