...
Current product version: `0.1.0.82`
...
Current workstream: version 0.1.0.82 is published as the startup-load hotfix for current live game data. Progress Atlas and Automation now validate and safely skip incomplete dynamic duty-catalog references instead of throwing during plugin construction or later eligibility scans. Version 0.1.0.81 restored destination-aware arrival selection for cross-zone saved routes and One Click Navigation; version 0.1.0.80 completed that feature's transactional shortcut handoff. The overall consolidation remains active and must continue against `VieriFeatureParity` and the predecessor behavior list rather than waiting for user-discovered omissions.
