...
2. Live-test the 0.1.0.32 gear transaction once: Start/Resume should show a bounded gear task, respect the configured gil floor, restore the provider's prior smart-shopping settings, and proceed to one duty only after verification. Test Stop during gear work only if the transaction runs long enough naturally; do not manufacture repeated micro-gates.
