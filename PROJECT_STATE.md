...
- `UI/NexusWindow.cs` — entire current shell and pages, including the standalone one-scrollbar Command Center with filters, favorites, hidden restoration, command selection, preferred/custom actions, and behavior/hotkey controls.
...
There are 291 automated tests across the application/domain policies, including `NexusCommandPolicyTests.cs` for aliases, public-version alignment, payload bounds, character scoping, and rejected unsafe controls, `AutoDutyProviderIdentityPolicyTests.cs` for stock, Vieri compatibility, disabled, and duplicate-loaded identity states, `QuestionableProviderIdentityPolicyTests.cs` for the corresponding quest-provider handoff states, and Command Center importer/atomic-working-copy coverage.
...
- **VieriDeck -> Command Center:** IMPLEMENTED in 0.1.0.53 as a Nexus-owned command palette/catalog with complete transactional configuration import, atomic editable working state, rollback, fresh initialization, filters/favorites/hidden entries, preferred/custom commands, editable exact-modifier hotkey, quick opening/toggling/settings, attached command panel, and preserved layout reference. VieriDeck remains installed only through the ordinary acceptance/rollback gate, after which deliberate package/feed retirement can proceed.
...
- Plugin project version: `0.1.0.54`. Dalamud API 15. Version 0.1.0.54 is published through the production feed.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
