...
Current published release source: `0.1.0.43`, commit `a993710552360b88b6032ed6ad488b31d7de9e98`. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.43 is live with native Mapping/Remapping exploration completion, the complete 666-target Hunting/Grand Company catalog, exact live per-target kill counts, current-rank target display, and provider-neutral next-target selection. The next large slice connects that owned model to narrow stock Lifestream, vnavmesh, and BossMod mechanics with Nexus-owned Stop/recovery before enabling Hunting Log execution.
...
- `Services/NexusMaintenanceRuntimeService.cs` — Nexus-owned bounded self-repair, materia extraction, collectible registration, and eligible-coffer runner with current-character profile selection, exclusive UI/inventory leases, completion verification/retry, gearset restoration, timeouts, and unified Stop. It invokes no AutoDuty IPC and deliberately excludes destructive selling/desynthesis/turn-in/storage work.
...
- `Services/ProgressionProviderService.cs` — VieriCodex, stock Questionable, VieriAutoDuty, and stock AutoDuty presence plus IPC-contract probe. Nexus enumerates and classifies current game-data Class/Job/Role and ordinary general side quests, applies current-job and provider eligibility, dispatches one exact quest, observes its exact ID/completion, session-blacklists pathless candidates, and exposes bounded Stop. The duty edge exposes one exact bounded `Run`, status/path observations, and `Stop`; it resets provider leveling mode through either direct `SetLevelingMode` or stock `SetConfig`, distinguishes stock/transition flavor, and rejects simultaneous ready quest providers. Gear planning and execution delegate only to Nexus-owned services; no AutoDuty gear IPC remains.
...
- `UI/NexusOperationsOverlay.cs` — optional compact Goto/Gear/Inventory/Extras surface over working Nexus controls with current operation status and a unified Stop action.
- `UI/NexusTheme.cs` — dark/red/gold ImGui theme and shared status/section helpers.

### `tests/VieriNexus.Application.Tests`

There are 251 automated tests across:
...
The published 0.1.0.43 source passes all 251 tests plus a zero-warning full plugin build.
...
`ReachJobLevelPlanner` is the first concrete desired-state planner. It validates current character/job/level, target level, a hard non-negative gil reserve, at least one allowed leveling method, and compatible provider availability. It can propose Nexus-owned gear readiness, one exact Class/Job/Role or ordinary general side quest, exactly one bounded duty, and post-activity level verification/replanning. An unavailable optional lane is a warning when another allowed lane is ready; no ready provider or an ambiguous double-provider state blocks the draft. Gear readiness, both exact quest families, and bounded duties are connected to execution. Hunting Log remains visible but explicitly unconnected.

The current character's draft persists in Nexus configuration. `ProgressionExecutionCoordinator` and `FileProgressionGoalStore` durably own one character-scoped Reach Job Level desired state, plan revision, bounded task history, active task, Stop-after flag, provider-start checkpoint, duty-entry checkpoint, and matching game duty-completion checkpoint. Nexus first creates a separate native gear-readiness task, owns its travel/UI/inventory resources, enforces the gil policy, and verifies live item level/gil. It then prefers one exact eligible Class/Job/Role quest, followed by one exact ordinary general side quest, pins the quest ID/kind, and delegates `StartSingleQuest` through the selected Questionable-compatible contract; a false start marks that provider/quest path unsupported for the session and automatically selects the next candidate, capped at 24 fallbacks. When no supported quest is available it selects one eligible unlocked duty and calls only stock-compatible bounded AutoDuty IPC. Each operation returns to Nexus for verification and replanning. Stop, Stop-after, provider loss/mismatch, job change, reload, and unload are reconciled through the active task's provider without replaying stale work. There is still no general cross-module goal repository/task graph executor; Hunting Log remains unfinished.
...
- **Hunting Log execution (next slice)** — observation, the full catalog, progress, and target selection are implemented. Execution remains deliberately unavailable until stock movement/combat boundaries are wired with Nexus-owned Stop and recovery.
...
- Plugin project version: `0.1.0.43`. Dalamud API 15. Version 0.1.0.43 is published and live through the production Dalamud feed.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
...
**Partially complete:** world state, dependency health, module contract, IPC surface, configuration migration framework, ownership, solo-duty policy, navigation migration, Progression execution, and operations migration. Progression duties, native Gear & Inventory, the safe native maintenance subset, install-first NavPlotter/AutoDuty migration, striking-dummy travel, and the preserved custom overlay are implemented. Quest execution, destructive maintenance, storage, in-duty withdrawal, remaining predecessor importers, and final stock-provider retirement are not.

**Not implemented:** automatic replay of interrupted movement (intentionally forbidden); durable general audit/history beyond the character-scoped Progression task document; the other seven predecessor transactional importers; Communications secret adapter; general multi-module scheduler/executor/reconciler; command gateway/event bus; SQLite history; diagnostics/support export; legacy IPC aliases; migrated quest/combat/market/communications/command-center/custom-UI runtimes; destructive maintenance/storage/in-duty withdrawal; standalone retirement.
...
2. **NEXT SUBSTANTIAL SLICE — Progression and stock providers:** freeze the Vieri-specific behavior currently carried by VieriCodex into Nexus-owned planning/policy/UI capabilities; introduce the bounded stock-Questionable adapter; keep the Vieri quest fork authoritative until parity is proven.
3. **AUTODUTY DECOUPLING:** Routes, Gear & Inventory, Progression scheduling/Last Run, safe maintenance, striking-dummy travel, operations settings, and the compact overlay are now Nexus-owned. Finish destructive maintenance/storage/in-duty withdrawal, telemetry/command parity, and the remaining duty-engine audit while keeping the stock duty engine external and updateable.
...
8. Connect the Nexus-owned Hunting/Grand Company model to narrow stock Lifestream, vnavmesh, and BossMod mechanics with explicit Stop, lease, target, kill-verification, reload, and provider-loss recovery. Do not route Hunting Logs through ordinary Questionable quest IPC. Then finish destructive maintenance/storage and remaining module migrations in similarly broad ownership slices.
...
Verification evidence for 0.1.0.43: Nexus source `a993710552360b88b6032ed6ad488b31d7de9e98` and Daily Pilcrow release `eccc72a8ee2f04f5cea3689799aa439f7ea4aa28` are pushed. All 251 Nexus tests, the zero-warning clean Release build, all 205 website tests, typecheck, focused package validation, thirteen-entry inventory guard, and production website build pass. Release deployment `dpl_BhUZV1nUxVTmoiHwRzAwKH5CUCBp` is Ready on `https://thedailypilcrow.com`. The live feed advertises VieriNexus 0.1.0.43; public runtime/source downloads return HTTP 200 as valid ZIPs and match SHA-256 `A0C8711924C1751E69D8E4C69EC4043EAFC6D4F43DAB6B05B126332B33C52928` / `61A6A7F9EC0B286C1179CD7D741EB37E61695D7CB2A98715FD35A0B600D025DE`. GitHub Actions Discord workflow `34600837053` completed successfully. Nexus now owns Mapping/Remapping exploration completion, the complete 12-log/666-target Hunting and Grand Company catalog, exact live per-target kill progress, current incomplete-target display, and provider-neutral deterministic next-target selection. Actual travel/combat remains disabled until its narrow stock-provider contracts and Stop/recovery are complete.
