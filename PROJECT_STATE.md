...
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.51`, assembly/internal root `VieriNexus`.
...
There are 280 automated tests across the application/domain policies, including `NexusCommandPolicyTests.cs` for aliases, public-version alignment, payload bounds, character scoping, and rejected unsafe controls, `AutoDutyProviderIdentityPolicyTests.cs` for stock, Vieri compatibility, disabled, and duplicate-loaded identity states, and `QuestionableProviderIdentityPolicyTests.cs` for the corresponding quest-provider handoff states.
...
- `/vierinexus`
- `/nexus`
- Page subcommands: `home`, `splash` (alias of Home), `show`, `hide`, `dependencies`, `migration`, `routes`, `progression`, and `atlas`.
...
- **Current VieriCodex integration:** version 1.12.2.81 at `1bc749b1e6f04519608b04caaca21c882e28412f` incorporates Questionable 15.756.2.4 at `f1b6da5ee5a9509c2f955f1fa93e73d5bfc4ffb8`. It includes the 305-file quest/path/IPC/navigation/UI update while preserving the Vieri live-gearset filter and empty-result guard, Progress Atlas, Progression Queue, named routes, solo-duty coordination, branding, and saved settings. The exact final-tree inventory and retirement gates are in `docs/QUESTIONABLE_PROVIDER_MIGRATION_AUDIT.md`.
...
10. Treat 0.1.0.51 as the equivalent Questionable handoff boundary: VieriCodex and stock Questionable may never both delegate work, stock is selected automatically after the fork is disabled, and the final-tree/protected-regression inventory lives in `docs/QUESTIONABLE_PROVIDER_MIGRATION_AUDIT.md`. Future compatible Questionable updates should require a contract/content audit, not a Nexus code change or permanent Vieri fork merge.
