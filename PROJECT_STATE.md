...
Current product version: `0.1.0.60`
Current published source: `0.1.0.60`, source `9ab70eaa8e00ae7866738432b3f97e8d3521428e`, website release `5adaeb22669132e8d6205b90aead0221db644189`, verification documentation `4642e9b58d91168e1cf49d3c13764e00873a13ec`. The unrelated untracked `rustdesk-1.4.9-x86_64.exe` remains untouched.
Current workstream: version 0.1.0.60 is the published player-facing class/job typography release. Every displayed full name is normalized to title case (`Red Mage`, `Black Mage`, `Warrior`) and every abbreviation remains uppercase (`RDM`, `BLM`, `WAR`); only the hidden Fast Job Switcher slash command stays lowercase as required by that provider. Version 0.1.0.59 remains the native multi-job queue and final VieriCodex runtime-retirement foundation. Nexus owns every imported queue step, target, method, fallback, advancement setting, restart state, and completion checkpoint per character. Existing verified imports are promoted once automatically; new/friend computers still install Nexus and import locally before disabling predecessors. Stock Questionable is the only quest runtime candidate; VieriCodex is detected only as a migration-source conflict and receives no runtime IPC calls. The next broad consolidation slice is the remaining AutoDuty provider/recovery retirement boundary. Separately, stock AutoDuty 0.0.0.335's remaining generic recovery difference is isolated with five passing policy tests on branch `nexus-recovery-fixes-0.0.0.335` at `2c583870c216573a054af109d11fafdd85d556be`.
...
- `VieriNexus.Plugin.csproj` — `Dalamud.NET.Sdk/15.0.0`, version `0.1.0.60`, assembly/internal root `VieriNexus`.
...
The 0.1.0.60 release candidate passes all 323 tests plus a zero-warning full plugin build.
...
- Plugin project version: `0.1.0.60`. Dalamud API 15. Version 0.1.0.59 remains published through the production feed until the 0.1.0.60 release completes.
- Production Dalamud custom-repository URL: `https://www.thedailypilcrow.com/dalamud/pluginmaster.json`.
