...
Current product version: `0.1.0.87`
...
Current workstream: version 0.1.0.87 restores the complete VieriAutoDuty outdoor movement handoff that the Nexus-native vendor trip had only partially reproduced. After Lifestream reaches an outdoor vendor territory, Nexus checks the character's actual Aether Current completion for that zone, mounts, confirms takeoff, and only then dispatches the flying vnavmesh leg. Locked flight immediately uses a ground route, and failed mount/takeoff preparation automatically falls back to ground after twelve seconds instead of parking at the arrival Aetheryte. This fixes the Anagnorisis level-86 gear trip and applies to every outdoor Nexus route. Version 0.1.0.86's one-click Gear -> Shop for Upgrades action and all prior protected shopping, maintenance, Atlas, and provider behavior remain intact. The overall consolidation remains subject to the non-negotiable source-to-source parity gate below.
