...
- Source provenance now includes VieriAutoDuty 1.0.0.426 and VieriNavPlotter 1.0.0.9, preserving the complete manual Shop for Upgrades transaction, the validated route playback/travel contract, explicit visualization ownership for suite routes and vendor shopping, and hidden ordinary Goto/dungeon navigation. Automatic/manual shopping and NavPlotter vendor playback carry the same vendor identity and NPC coordinate, but a visible NPC is now considered reached only when FFXIV's native interaction check succeeds; rounded distance and coordinates cannot overrule a native out-of-range result. The coordinate fallback is limited to 3.1 yalms while the object is unavailable. Geraint uses the measured walkable point `Territory 133 | X 168.4092 | Y 15.6999 | Z -73.9508`, with his behind-counter coordinate retained only for lookup and native interaction checks.
