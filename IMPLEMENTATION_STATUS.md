...
- Source provenance now includes VieriAutoDuty 1.0.0.424, preserving the complete manual Shop for Upgrades transaction, the validated route playback/travel contract consumed by VieriNavPlotter, explicit visualization ownership for suite routes and vendor shopping, hidden ordinary Goto/dungeon navigation, and an in-loop vendor-arrival handoff that stops gear-shopping, selling, repair, card-exchange, and GC turn-in movement inside normal interaction range. It uses live Event NPC/hitbox state when available and a 4.25-yalm trusted-coordinate fallback on only the final vendor approach when that state is delayed, preventing counter collisions without skipping authored waypoints.
