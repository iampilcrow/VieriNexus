...
- Source provenance now includes VieriAutoDuty 1.0.0.427 and VieriNavPlotter 1.0.0.9, preserving the complete manual Shop for Upgrades transaction, the validated route playback/travel contract, explicit visualization ownership for suite routes and vendor shopping, and hidden ordinary Goto/dungeon navigation. Distinct authored standing points always run to completion before native interaction eligibility can end travel; raw NPC-coordinate routes retain native early stopping before collision. Rounded distance and coordinates cannot overrule a native out-of-range result, and the 3.1-yalm fallback applies only while the NPC object is unavailable. Geraint uses the measured walkable point `Territory 133 | X 168.4092 | Y 15.6999 | Z -73.9508`, with his behind-counter coordinate retained only for lookup and interaction.
