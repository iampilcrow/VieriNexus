...
- Source provenance now includes VieriAutoDuty 1.0.0.425 and VieriNavPlotter 1.0.0.8, preserving the complete manual Shop for Upgrades transaction, the validated route playback/travel contract, explicit visualization ownership for suite routes and vendor shopping, hidden ordinary Goto/dungeon navigation, and an in-loop vendor-arrival handoff that stops movement inside normal interaction range. Automatic/manual shopping and NavPlotter's built-in Play Route/Travel to Destination now carry the same vendor target identity and trusted NPC coordinate; live Event NPC/hitbox state remains primary and a 4.25-yalm coordinate fallback applies only on the final vendor approach, preventing counter collisions without skipping authored waypoints. Multi-point Travel to Start and generic named routes deliberately omit final-vendor completion metadata.
