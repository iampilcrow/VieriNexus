...
15. Built-in route migration must preserve provenance and certainty: destination-only AutoDuty entries may not be presented as complete authored paths, while corrected measured approaches and the Old Sharlayan stair coordinate set remain visible, copyable references. Domitien's wall-side NPC coordinate is forbidden as a movement point; navigation must approach through `Territory 133 | X 164.4264 | Y 15.5000 | Z -75.7035` and finish at `X 157.5930 | Y 15.7000 | Z -69.3316`. Geraint must route to the measured walkable point `X 168.4092 | Y 15.6999 | Z -73.9508`; his behind-counter object coordinate remains lookup metadata only. A visible NPC is reached only when FFXIV's native interaction check succeeds. Rounded distance and coordinates may never overrule a native out-of-range result; the conservative 3.1-yalm coordinate fallback is allowed only while the NPC object is unavailable.
