...
4. **Automatic transaction live-accepted in 0.1.0.32; manual preview/approval implemented in 0.1.0.33:** Nexus creates the durable pre-duty gear task, owns its full resources and gil floor, verifies the outcome, then selects one duty. The accepted live run bought gear, returned to the inn, and started the duty. The new Gear & Inventory page owns the manual review, selected slots, exact item/quantity/maximum-price approval, spending floor, full-run leases, and Stop. VieriAutoDuty 1.0.0.441 supplies only a versioned read-only live candidate scan and rejects approvals that no longer match its freshly rebuilt plan before using the existing vendor/equip mechanics. Continue moving vendor-band/catalog candidate generation, low-level slot/equip/gearset/displaced-item mechanics, maintenance, and the rest of the categorized overlay into Nexus with golden configuration and incident fixtures.
