...
4. **Automatic transaction accepted in 0.1.0.32; manual approval accepted in 0.1.0.33; ranking moved in 0.1.0.34; live catalog ownership moved in 0.1.0.35:** Nexus creates the durable pre-duty gear task, owns its full resources and gil floor, verifies the outcome, then selects one duty. Gear & Inventory now reads current and owned equipment, resolves only real ordinary gil shops attached to the curated vendors, selects the vendor band/job family, ranks replacements, protects EXP and two-handed layouts, and creates the exact automatic or manual approval. The automatic lane no longer invokes VieriAutoDuty's gear planner, and the manual preview no longer invokes its raw snapshot. VieriAutoDuty retains only physical shop-window, purchase, verified-equip, gearset-update, displaced-equipment, and fresh-plan safety mechanics. Move those remaining actions together, then maintenance and the rest of the categorized overlay, with golden configuration and incident fixtures.
