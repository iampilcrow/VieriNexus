...
3. **Implemented in 0.1.0.30, compatibility-corrected in 0.1.0.31, and live-accepted:** build the Progression duty proof using the stock-compatible AutoDuty contract. Nexus selects one exact eligible duty, disables AutoDuty's internal leveling scheduler through either `SetLevelingMode` or the older stock `SetConfig("leveling", "None")` path, requests one loop, holds the full resource bundle, requires the matching game duty-completed event plus safe return, and decides whether to stop or schedule exactly one fresh task. It never calls the Vieri-only endless Progression endpoint. The user accepted Start, Stop, fresh-plan Resume, and repeated Mt. Gulg dispatch. Version 0.1.0.39 adds exact Class/Job/Role quest selection and execution through stock Questionable's existing single-quest IPC contract; VieriCodex remains only a temporary same-contract fallback while Hunting Log and general side-quest selection migrate.
...
7. **Protected item transactions implemented in 0.1.0.47:** Nexus owns imported ordering, exact selling selection/approval, gearset/EXP/collectable protection, desynthesis filtering, resource leases, Stop, timeout, and completion observation. It invokes no AutoDuty maintenance endpoint. AutoRetainer remains a narrow Grand Company mechanics provider and Glamour Log remains a narrow eligible-storage mechanics provider. In-duty withdrawal stays inactive rather than treating AutoDuty Stop as a fake leave/resume contract.
