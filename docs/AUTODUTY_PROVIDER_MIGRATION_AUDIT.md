...
4. **Automatic transaction accepted in 0.1.0.32; manual approval accepted in 0.1.0.33; candidate ranking moved in 0.1.0.34:** Nexus creates the durable pre-duty gear task, owns its full resources and gil floor, verifies the outcome, then selects one duty. The automatic live run bought gear, returned to the inn, and started the duty. The manual live MCH scan correctly reported zero upgrades without movement or spending. Gear & Inventory owns the review, role-aware per-slot candidate ranking, EXP and two-handed/off-hand policy, selected slots, exact item/quantity/maximum-price approval, spending floor, full-run leases, and Stop. VieriAutoDuty 1.0.0.442 now reports raw live equipment and catalog facts and retains a fresh-plan safety recheck before using the existing vendor/equip mechanics. Continue moving vendor-band/catalog discovery, low-level purchase/equip/gearset/displaced-item mechanics, maintenance, and the rest of the categorized overlay into Nexus with golden configuration and incident fixtures.
