...
namespace DelvUI
{
    public class Plugin : IDalamudPlugin
    {
        ...
        public string Name => "VieriDelvUI";
            ...
            ISeStringEvaluator seStringEvaluator)
        {
            ...
            EnemyMarkerIconManager.Initialize();
            HighlightingManager.Initialize();
            ...
            CommandManager.AddHandler(
                ...
                {
                    HelpMessage = "Opens the VieriDelvUI configuration window.\n"
                                + "/vieridelvui toggle → Toggles HUD visibility.\n"
                                + "/vieridelvui show → Shows HUD.\n"
                                + "/vieridelvui hide → Hides HUD.\n"
                                + "/vieridelvui toggledefaulthud → Toggles the game's Job Gauges visibility.\n"
                                + "/vieridelvui forcejob <JOB> → Forces VieriDelvUI to show the HUD for the given Job short name.\n"
                                + "/vieridelvui profile <PROFILE> → Switch to the given profile.\n"
                                + "/vieridelvui mouse <on/off> → Toggles special input handling to support extra mouse buttons when hovering VieriDelvUI elements.",
            ...
            CommandManager.AddHandler(
                ...
                {
                    HelpMessage = "Opens the VieriDelvUI configuration window.\n"
                                + "/vdui toggle → Toggles HUD visibility.\n"
                                + "/vdui show → Shows HUD.\n"
                                + "/vdui hide → Hides HUD.\n"
                                + "/vdui toggledefaulthud → Toggles the game's Job Gauges visibility.\n"
                                + "/vdui forcejob <JOB> → Forces VieriDelvUI to show the HUD for the given Job short name.\n"
                                + "/vdui profile <PROFILE> → Switch to the given profile.\n"
                                + "/vdui mouse <on/off> → Toggles special input handling to support extra mouse buttons when hovering VieriDelvUI elements.",
        ...
        {
            string bannerImage = Path.Combine(Path.GetDirectoryName(AssemblyLocation) ?? "", "Media", "Images", "VieriDelvUI.png");
            ...
            else
            {
                ...
                switch (arguments)
                {
                    ...
                    case { } argument when argument.StartsWith("mouse"):
                        ...
                        Chat.Print($"VieriDelvUI special mouse handling is currently {mouseStr}.");
                        break;
                ...
                if (printHUDStatus)
                {
                    ...
                    Chat.Print($"VieriDelvUI HUD is {hudStr}.");
                }
            }
        }
        ...
        private void Draw()
        {
            ...
            EnemyMarkerIconManager.Instance?.Update();
            HighlightingManager.Instance?.Update();
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            Logger.Info("\tDisposing EnemyMarkerIconManager...");
            EnemyMarkerIconManager.Instance?.Dispose();

            Logger.Info("\tDisposing HighlightingManager...");
            HighlightingManager.Instance?.Dispose();
            ...
            CommandManager.RemoveHandler("/vieridelvui");
            CommandManager.RemoveHandler("/vdui");
