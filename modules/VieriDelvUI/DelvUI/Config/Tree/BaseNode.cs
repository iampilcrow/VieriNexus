        ...
        public void Draw(float alpha)
        {
            ...
            ImGui.BeginGroup(); // Middle section
            {
                ImGui.BeginGroup(); // Left
                {
                    ...
                    {
                        // Keep the banner at the original fixed pixel footprint. Scaling it
                        // with the configuration UI pushed only the left navigation downward.
                        Vector2 maximumBannerSize = new(145, 50);
                        float bannerScale = MathF.Min(maximumBannerSize.X / delvUiBanner.Width, maximumBannerSize.Y / delvUiBanner.Height);
                        Vector2 bannerSize = new(delvUiBanner.Width * bannerScale, delvUiBanner.Height * bannerScale);
                        ImGui.SetCursorPos(new Vector2(15 + (150 * _scale - bannerSize.X) / 2f, 5));
                        ImGui.Image(delvUiBanner.Handle, bannerSize);
                    }

                    // version
                    string versionText = $"v{Plugin.Version}";
                    float versionWidth = ImGui.CalcTextSize(versionText).X;
                    ImGui.SetCursorPos(new Vector2(15 + (150 * _scale - versionWidth) / 2f, 40));
