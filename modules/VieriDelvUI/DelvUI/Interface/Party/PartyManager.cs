...
namespace DelvUI.Interface.Party
{
    ...
    public unsafe class PartyManager : IDisposable
    {
        ...
        private int _realMemberCount
        {
            get
            {
                PartyListNumberArray* partyList = PartyListNumberArray.Instance();
                return partyList != null ? partyList->PartyListCount : PartyListAddon != null ? PartyListAddon->MemberCount : Plugin.PartyList.Length;
            }
        }

        private int _trustMemberCount
        {
            get
            {
                PartyListNumberArray* partyList = PartyListNumberArray.Instance();
                return partyList != null ? partyList->TrustCount : PartyListAddon != null ? PartyListAddon->TrustCount : 0;
            }
        }
        ...
        private void InternalUpdate()
        {
            ...
            try
            {
                ...
                // solo
                if (_realMemberCount <= 1 && _trustMemberCount == 0)
                {
        ...
        private Dictionary<string, InternalMemberData> GetMembersDataMap(IPlayerCharacter player, bool isCrossWorld)
        {
            ...
            if (isCrossWorld)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    AddMemberData(dataMap, new InternalMemberData
                    {
                        Index = i,
            ...
            {
                // The number array is the authoritative source for both player and
                // Duty Support/Trust rows. The former private HudAgent offset changes
                // between game builds and can return an empty list even while the
                // native Light Party window is populated.
                PartyListNumberArray* numberArray = PartyListNumberArray.Instance();
                if (numberArray != null)
                {
                    int partyCount = Math.Clamp(numberArray->PartyListCount, 0, 8);
                    for (int i = 0; i < partyCount; i++)
                    {
                        PartyListNumberArray.PartyListMemberNumberArray member = numberArray->PartyMembers[i];
                        string name = PartyListAddon->PartyMembers[i].Name != null
                            ? PartyListAddon->PartyMembers[i].Name->NodeText.ToString()
                            : string.Empty;
                        name = ResolvePartyMemberName(name, member.EntityId, player);

                        AddMemberData(dataMap, new InternalMemberData
                        {
                            Index = i,
                            ObjectId = member.EntityId,
                            ContentId = ResolvePartyMemberContentId(name),
                            Name = name,
                            Order = member.DisplayRow >= 0 ? member.DisplayRow : i
                        });
                    }

                    int trustCount = Math.Clamp(numberArray->TrustCount, 0, 7);
                    for (int i = 0; i < trustCount; i++)
                    {
                        PartyListNumberArray.PartyListMemberNumberArray member = numberArray->TrustMembers[i];
                        string name = PartyListAddon->TrustMembers[i].Name != null
                            ? PartyListAddon->TrustMembers[i].Name->NodeText.ToString()
                            : string.Empty;
                        ...
                        AddMemberData(dataMap, new InternalMemberData
                        {
                            ...
                            ObjectId = member.EntityId,
                            Name = ResolvePartyMemberName(name, member.EntityId, player),
                            Order = member.DisplayRow >= 0 ? member.DisplayRow : partyCount + i
                        });
                    }
                }

                // Retain the legacy reader only as a compatibility fallback for a
                // future client layout where the structured arrays are unavailable.
                if (dataMap.Count == 0 && HudAgent != IntPtr.Zero)
                {
                    int count = _realMemberCount + _trustMemberCount;
                    for (int i = 0; i < count; i++)
                    {
            ...
            // status string
            int visibleCount = dataMap.Count;
        ...
        private static string ResolvePartyMemberName(string uiName, uint entityId, IPlayerCharacter player)
        {
            ...
            return Plugin.ObjectTable.SearchById(entityId)?.Name.ToString() ?? $"PartyMember_{entityId:X8}";
        }

        private long ResolvePartyMemberContentId(string name)
        {
            (DalamudPartyMember?, int)? member = GetDalamudPartyMember(name);
            return member?.Item1 != null ? (long)member.Value.Item1.ContentId : 0;
        }

        private static void AddMemberData(Dictionary<string, InternalMemberData> dataMap, InternalMemberData data)
        {
            string key = data.Name;
            if (string.IsNullOrWhiteSpace(key))
            {
                key = $"PartyMember_{data.ObjectId:X8}_{data.Index}";
                data.Name = key;
            }

            if (dataMap.ContainsKey(key))
            {
                key = $"{key}_{data.ObjectId:X8}_{data.Index}";
            }
        ...
        private void UpdateTrustParty(IPlayerCharacter player, Dictionary<string, InternalMemberData> dataMap, bool forced)
        {
            ...
            if (softUpdate)
            {
                foreach (IPartyFramesMember member in _groupMembers)
                {
                    ...
                    {
                        member.Update(EnmityForTrustMemberIndex(member.Index - _realMemberCount), PartyMemberStatus.None, ReadyCheckStatus.None, false, 0);
                    }
                }
            }
            else
            {
                ...
                for (int i = 0; i < keys.Length; i++)
                {
                    ...
                    if (data.ObjectId == player.GameObjectId || keys[i] == player.Name.ToString())
                    {
                        PartyFramesMember playerMember = new PartyFramesMember(player, data.Index, data.Order, EnmityForIndex(data.Index), PartyMemberStatus.None, ReadyCheckStatus.None, true);
                        _groupMembers.Add(playerMember);
                    }
                    else
                    {
                        // Prefer the entity id supplied by the native party list. Names can
                        // differ between the UI row and object table, and Trust/Duty Support
                        // actors are not guaranteed to occupy the even object-table slots.
                        ICharacter? trustChara = Plugin.ObjectTable.SearchById(data.ObjectId) as ICharacter
                            ?? Utils.GetGameObjectByName(keys[i]) as ICharacter;
                        if (trustChara != null)
                        {
                            _groupMembers.Add(new PartyFramesMember(trustChara, data.Index, data.Order, EnmityForTrustMemberIndex(data.Index - _realMemberCount), PartyMemberStatus.None, ReadyCheckStatus.None, false));
                        }
                    }
                }
            }
