...
namespace DelvUI.Interface
{
    public class HudManager : IDisposable
    {
        ...
        public void Draw(uint jobId)
        {
            ...
            // nameplates and highlighting guide lines
            bool drawNameplates = _nameplatesHud.GetConfig().Enabled;
            bool drawHighlightingLines = HighlightingManager.Instance?.HasLines == true;
            if (drawNameplates || drawHighlightingLines)
            {
                ClipRectsHelper.Instance?.BeginNameplatesClipping();
                try
                {
                    ClipRectsHelper.Instance?.AddNameplatesClipRects();

                    if (drawHighlightingLines)
                    {
                        HighlightingManager.Instance?.DrawLines();
                    }
                ...
                finally
                {
                    ...
                    ClipRectsHelper.Instance?.EndNameplatesClipping();
                }
            }
