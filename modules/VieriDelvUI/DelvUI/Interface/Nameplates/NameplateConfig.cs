...
namespace DelvUI.Interface.GeneralElements
{
    ...
    public enum EnemyNameplateLayoutMode
    {
        Overlap = 0,
        Stack = 1
    };
    ...
    public sealed class ResetEnemyMarkerTransformAttribute : ConfigAttribute
    {
        public ResetEnemyMarkerTransformAttribute(string friendlyName) : base(friendlyName)
        {
        }

        public override bool DrawField(FieldInfo field, PluginConfigObject config, string? ID, bool collapsingHeader)
        {
            if (!ImGui.Button(friendlyName + IDText(ID)))
            {
                return false;
            }

            if (config is EnemyMarkerIconConfig markerConfig)
            {
                markerConfig.OffsetX = 0f;
                markerConfig.OffsetY = 0f;
                markerConfig.Scale = 1f;
            }
    ...
    {
        [Combo("Layout Mode", new string[] { "Overlapping", "Stacking" }, help = "Overlapping keeps enemy nameplates at their original screen positions.\n\nStacking separates hostile enemy nameplates vertically when they would cover one another.")]
        [Order(5)]
        public EnemyNameplateLayoutMode LayoutMode = EnemyNameplateLayoutMode.Overlap;

        [DragInt("Stack Spacing", min = 20, max = 150, help = "The vertical distance in pixels between stacked enemy nameplates.")]
        [Order(6)]
        public int StackSpacing = 45;

        [DragInt("Stack Grouping Width", min = 50, max = 800, help = "Enemy nameplates closer than this horizontal distance are treated as overlapping candidates. Increase this if wide nameplates still cover one another.")]
        [Order(7)]
        public int StackGroupingWidth = 240;
    ...
    [DisableParentSettings]
    public class EnemyMarkerIconConfig : PluginConfigObject
    {
        // Retained as serialized migration state for users who loaded 2.7.0.9.
        // The visible control is now the standard nested Enabled checkbox so this
        // page receives the same collapsible header as Icon, Debuffs, and Castbar.
        public bool AdjustMarkerIcons = true;

        public int MarkerConfigVersion;

        [DragFloat("Horizontal offset", min = -300f, max = 300f, velocity = 1f, help = "Moves the marker left or right from its normal nameplate position, in pixels.")]
        [Order(10)]
        public float OffsetX;

        [DragFloat("Vertical offset", min = -300f, max = 300f, velocity = 1f, help = "Moves the marker up or down from its normal nameplate position, in pixels.")]
        [Order(11)]
        public float OffsetY;

        [DragFloat("Size multiplier", min = 0.25f, max = 5f, velocity = 0.01f, help = "Scales the native marker relative to its normal size.")]
        [Order(12)]
        public float Scale = 1f;

        [Checkbox("Render above nameplate elements", help = "Keeps the marker above the normal nameplate contents so it remains easy to see.")]
        [Order(20)]
        public bool RenderOnTop = true;

        [Checkbox("Force full opacity", help = "Prevents the native marker from becoming partially transparent.")]
        [Order(21)]
        public bool ForceFullOpacity = true;

        [JsonIgnore]
        [ResetEnemyMarkerTransform("Reset marker position and size", help = "Restores the marker's normal position and size without changing opacity or layering.")]
        [Order(30)]
        public bool ResetMarkerTransform;

        // Persisted migration metadata. These fields intentionally have no drawing
        // attributes, so they do not clutter the settings page.
        public bool ImportedFromVieriMarkerIcon;
    ...
    public class NameplateWithEnemyBarConfig : NameplateConfig, NameplateWithBarConfig
    {
        ...
        [NestedConfig("Enemy Marker Icon", 46)]
        public EnemyMarkerIconConfig MarkerIconConfig = new();
