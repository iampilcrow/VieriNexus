    ...
    private void ReceiveActionEffectDetour(ActionEffectSet set)
    {
        try
        {
            if(set.Source?.Address == Svc.Objects.LocalPlayer?.Address)
            {
                ...
                var positionalState = isPositional ? PositionalState.Unknown : PositionalState.Ignore;
                ...
                if(set.TargetEffects != null)
                {
                    foreach(var effect in set.TargetEffects)
                    {
                        effect.ForEach(entry =>
                        {
                            ...
                            {
                                var parameters = isPositional ? P.PositionalManager?.GetPositionalParameters(actionId, entry.param2) : null;
                                var matched = parameters?.IsHit == true;
                                if (isPositional)
                                    positionalState = global::Avarice.Positional.PositionalVerdict.Merge(positionalState, parameters?.IsHit);
