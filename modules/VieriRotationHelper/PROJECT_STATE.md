...
Current product version: `0.1.0.60` release candidate
...
Current workstream: version 0.1.0.60 is the player-facing class/job typography release candidate. Every displayed full name is normalized to title case (`Red Mage`, `Black Mage`, `Warrior`) and every abbreviation remains uppercase (`RDM`, `BLM`, `WAR`); only the hidden Fast Job Switcher slash command stays lowercase as required by that provider. Version 0.1.0.59 remains the published native multi-job queue and final VieriCodex runtime-retirement release. Nexus owns every imported queue step, target, method, fallback, advancement setting, restart state, and completion checkpoint per character. Existing verified imports are promoted once automatically; new/friend computers still install Nexus and import locally before disabling predecessors. Stock Questionable is the only quest runtime candidate; VieriCodex is detected only as a migration-source conflict and receives no runtime IPC calls. After 0.1.0.60, the next broad consolidation slice is the remaining AutoDuty provider/recovery retirement boundary. Separately, stock AutoDuty 0.0.0.335's remaining generic recovery difference is isolated with five passing policy tests on branch `nexus-recovery-fixes-0.0.0.335` at `2c583870c216573a054af109d11fafdd85d556be`.
