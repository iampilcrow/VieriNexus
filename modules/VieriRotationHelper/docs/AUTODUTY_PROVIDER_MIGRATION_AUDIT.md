...
- The remaining generic duty behavior is the fork's stale-path/death/re-entry recovery: it stops and clears an old vnavmesh path when territory/path execution is restarted, and chooses a safe authored restart index after death/shortcut recovery. Current stock `0.0.0.335` and current upstream development were checked directly and do not contain equivalent handling. The five-file generic correction is isolated on `nexus-recovery-fixes-0.0.0.335` at `2c583870c216573a054af109d11fafdd85d556be`; its dedicated recovery suite passes 5/5 and the stock Release build succeeds. It remains an upstream/stock concern rather than duty-engine code copied into Nexus.
