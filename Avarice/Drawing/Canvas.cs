...
internal unsafe class Canvas : Window
{
    ...
    {
        // Keep gameplay overlays hidden until the loaded world and player are stable.
        if (!P.GameplayOverlaysReady)
            return false;
    ...
    private void DrawAllOverlays(bool showPositionalFeatures)
    {
        ...
        if (P.currentProfile.CompassEnable && IsConditionMatching(P.currentProfile.CompassCondition))
        {
            static void DrawLetter(string l, Vector2 pos, Vector4? color = null)
            {
                ...
                if (P.config.HideBehindNativeUi && NativeUiOcclusion.IsCovered(pos - size / 2f, size))
                    return;
        ...
        if (P.currentProfile.EnablePlayerDot && IsConditionMatching(P.currentProfile.PlayerDotSettings.DisplayCondition))
        {
            ...
            if (NativeUiOcclusion.IsWorldPointCovered(Svc.Objects.LocalPlayer.Position, P.currentProfile.PlayerDotSettings.Thickness))
            {
                // Pictomancy's tiny dot primitive can bypass its native-UI mask, so suppress it explicitly.
            }
            else if (PictomancyRenderer.IsDrawing)
            {
        ...
        if (P.currentProfile.PartyDot && IsConditionMatching(P.currentProfile.PartyDotSettings.DisplayCondition))
        {
            foreach (var x in Svc.Party)
            {
                ...
                {
                    if (NativeUiOcclusion.IsWorldPointCovered(x.GameObject.Position, P.currentProfile.PartyDotSettings.Thickness))
                    {
                        continue;
                    }
                    else if (PictomancyRenderer.IsDrawing)
        ...
        if (P.currentProfile.AllDot && IsConditionMatching(P.currentProfile.AllDotSettings.DisplayCondition))
        {
            foreach (var x in Svc.Objects)
            {
                ...
                {
                    if (NativeUiOcclusion.IsWorldPointCovered(x.Position, P.currentProfile.AllDotSettings.Thickness))
                    {
                        continue;
                    }
                    else if (PictomancyRenderer.IsDrawing)
