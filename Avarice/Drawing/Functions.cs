...
internal static unsafe class Functions
{
    ...
    internal static void DrawAnticipatedPos(IBattleNpc bnpc)
    {
        ...
        // The integrated Wrath decision takes priority, including authoritative none.
        if (P.RotationHelperWatcher.TryGet(bnpc.GameObjectId, out var suiteSide))
        {
            if (suiteSide == 1) DrawRear();
            else if (suiteSide == 2) DrawSides();
            return;
        }
