...
namespace DelvUI.Helpers
{
    public class ClipRectsHelper
    {
        ...
        private int _nameplatesClippingDepth;

        private bool NameplatesClippingActive =>
            _nameplatesClippingDepth > 0 && _config.NameplatesClipRectsEnabled;

        public bool Enabled => _config.Enabled || NameplatesClippingActive;
        public WindowClippingMode? Mode => NameplatesClippingActive
            ? WindowClippingMode.Full
            : _config.Enabled ? _config.Mode : null;
        ...
        {
            // Nameplates have their own clipping option. Keep the native window
            // rectangles current for them even when general HUD clipping is off.
            if (!_config.Enabled && !_config.NameplatesClipRectsEnabled) { return; }
        ...
        public void BeginNameplatesClipping()
        {
            if (_config.NameplatesClipRectsEnabled)
            {
                _nameplatesClippingDepth++;
            }
        }

        public void EndNameplatesClipping()
        {
            if (_nameplatesClippingDepth > 0)
            {
                _nameplatesClippingDepth--;
            }
        }
