...
- Integrated DelvUI 2.8.0.0 while preserving VieriDelvUI branding, commands, profiles, highlighting, nameplate stacking, native enemy markers, and saved settings.
- Added Beastmaster job support to HUD visibility, job configuration, and role lookup.
- Updated distance reads and party ready-check handling for the current game and Dalamud APIs.

## 2.7.0.15

- Party Members highlighting now recognizes NPC companions from the native Trust/Duty Support party roster, even when their player-party status flag is absent.
- Role outlines use the companion's current party role: blue for tanks, green for healers, and red for DPS. Player job-role fallback remains available.
- Named NPC Custom rules retain priority, and party detection does not depend on Party Frames visibility or preview settings. Existing highlighting settings are unchanged.

## 2.7.0.14

- Added optional role highlighting to Player, Party Members, Alliance Members, Friends, and Other Players.
- Role highlighting colors tanks blue, healers green, and DPS red while preserving named Custom rule priority.

# 2.7.0.11
- Moved Reset marker position and size into the normal Enemy Marker Icon setting tree so it collapses with the section.
- Removed the legacy-import confirmation and visible-marker count lines from the settings page.
- The integrated native enemy marker icon now hides and restores with VieriDelvUI's master Hide HUD control.

# 2.7.0.10
- Changed the integrated Enemy Marker Icon settings to use the same expandable and collapsible header as the neighboring Icon, Debuffs, and Castbar sections.
- Preserved the enabled state saved by VieriDelvUI 2.7.0.9 while converting to the standard collapsible configuration layout.

# 2.7.0.9
- Prevented VieriDelvUI highlighting and guide lines from running before a character is fully logged into the world.
- Integrated VieriMarkerIcon controls into `Nameplates > Enemies > Enemy Marker Icon`, including offsets, scale, opacity, layering, reset controls, and visible-marker status.
- Added automatic one-time migration of existing VieriMarkerIcon settings while leaving the original settings file untouched as a backup.
- Added a compatibility handoff so VieriMarkerIcon and VieriDelvUI do not both modify the same native marker nodes during the transition.

# 2.7.0.8
- Added the shared Vieri logo to the Dalamud plugin listing without changing profiles or settings.

# 2.7.0.7
- Added an obvious Delete button directly on every Custom highlighting rule row.
- VieriDelvUI's master Hide HUD control now immediately clears and suppresses all native outlines and guide lines until the HUD is shown again.
- Native outlines are temporarily cleared while Character or Examine model-preview windows are visible, preventing world-player Highlight All settings from tinting preview models.

# 2.7.0.6
- Replaced the unreliable Objects Highlighting tab with a Custom tab for exact, case-insensitive named Player, Enemy, NPC, Pet, Minion, or Any rules, including a Use Current Target shortcut.
- Custom named rules take priority over normal Player, Party, Friend, Enemy, NPC, Pet, and Minion category settings and support Always or Current Target Only outline/guide-line modes.
- Removed the misleading custom model-outline wheel and now presents only the six outline colors FFXIV can reliably render.
- Changed category and Custom-rule guide lines to the same independent six-color dropdown for a consistent Highlighting interface; earlier custom line selections migrate to their nearest palette color.
- Migrated outline-wheel selections saved by 2.7.0.5 to their nearest real native outline color without resetting any other Highlighting settings.
- Removed FFXIV's Black outline choice because it produces a large magenta selection aura on many object types.
- Restored the original fixed banner footprint so the left navigation and right settings panel align at the top again across UI scales.
- Clarified that the native FFXIV outline effect does not expose per-object thickness control; guide-line thickness remains fully adjustable.

# 2.7.0.5
- Replaced the settings banner with the new VieriDelvUI logo and fitted it cleanly in the existing navigation area.
- Replaced the fixed highlighting-color list with full custom RGBA color controls in every Highlighting tab.
- Added configurable target-only and all-visible guide lines to every Highlighting category, including custom color and thickness controls.
- Preserved and automatically migrated all Highlighting settings created by VieriDelvUI 2.7.0.4.

# 2.7.0.4
- Added a new `Highlighting` settings section immediately after `Nameplates`.
- Added separate Player, Enemies, Party Members, Alliance Members, Friends, Other Players, Pets, NPCs, Minions, and Objects tabs.
- Each category can highlight only the current target, every visible object in that category, or both.
- Each category has an independent native FFXIV outline color selector.

# 2.7.0.3
- Added Overlapping and Stacking layout modes for hostile enemy nameplates.
- Added adjustable stack spacing and grouping width settings under `Nameplates > Enemies`.
# 2.7.0.1
