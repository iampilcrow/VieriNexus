...
Gear & Inventory now owns the manual Shop for Upgrades experience. It requests a read-only live candidate scan from the temporary VieriAutoDuty adapter, displays the current item and exact replacement for each slot, protects active EXP items, defaults to verified recommendations, and lets the user approve selected slots under the same hard gil reserve used by Progression. The approval pins the character, equipment signature, item IDs, quantities, and maximum unit prices; any change requires a fresh preview. The complete shopping run holds Nexus Teleport/Navigation/Movement/UI/Inventory leases through completion or confirmed Stop. VieriAutoDuty still supplies temporary vendor-window, purchase, verified-equip, gearset, and displaced-equipment mechanics while those lower-level operations migrate.
