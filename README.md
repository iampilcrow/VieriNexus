...
Command Center is now a complete Nexus-owned replacement for VieriDeck. Each computer can transactionally import its own plugin filters, favorites, hidden entries, selected command panel, preferred and custom commands, hotkey, and layout reference while leaving VieriDeck unchanged and rollbackable; a new user can also start fresh without any predecessor. Nexus discovers loaded and disabled plugins plus their registered commands, derives documented subcommands and Lifestream shortcuts, opens settings or main windows, safely toggles an already-open non-overlay window, runs a preferred quick action for command-only plugins, and lets the user add/remove custom commands and edit the hotkey. It remains available through `/nexus commands` and uses the page's single scrollbar.
