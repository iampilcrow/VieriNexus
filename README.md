...
The five remaining Vieri quest-route corrections are now a Nexus-managed compatibility layer over stock Questionable's complete downloaded bundle. Nexus checks exact route shapes and old values, creates a hash-addressed byte-identical official backup, replaces only the five required ZIP entries atomically, and reloads Questionable only after its public contract reports idle. An unexpected upstream change leaves the entire bundle untouched and blocks only an affected quest. New Questionable route data is rechecked automatically; native corrections are accepted without rewriting them. No upstream submission is used.
