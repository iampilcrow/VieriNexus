...
Plugins is the complete Nexus-owned replacement for VieriDeck. The page starts with Dalamud Plugins and Dalamud Settings, then places favorites above the remaining installed plugins. Each plugin expands its own registered and saved commands directly in place, with no second page scrollbar or distant command panel. Each computer can transactionally import its own filters, favorites, hidden entries, preferred/custom commands, hotkey, and layout reference while leaving VieriDeck unchanged and rollbackable; a new user can also start fresh. Open it with `/nexus plugins`; the older `/nexus commands` alias remains compatible.
