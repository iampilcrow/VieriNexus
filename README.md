...
Progression now owns a durable gear-and-duty execution lifecycle. For the current job, it persists a target level, allowed job-quest/Hunting Log/side-quest/duty methods, a hard minimum-gil reserve, bounded task history, completion checkpoints, Last Run, Stop, and plan revision in character-scoped atomic storage. Before selecting a duty, Nexus owns a separate gear-readiness transaction, temporarily applies the approved gil floor to VieriAutoDuty's vendor/equip mechanics, restores its prior settings, and verifies live gil and item level. Nexus then asks the selected AutoDuty implementation to run exactly one eligible duty, verifies the matching game duty-completion event and return to the world, and either satisfies, pauses, blocks, or schedules one fresh task. Quest execution and the remaining detailed shopping policy/UI are still being migrated.

Gear & Inventory now owns the manual Shop for Upgrades experience and upgrade selection. It receives raw live equipment and ordinary gil-vendor catalog facts from the temporary VieriAutoDuty adapter, scores the current job's primary stat, chooses the winning item per slot, protects active EXP items, rejects off-hand purchases behind a two-handed weapon, and displays the exact replacements it selected. The user approves selected slots under the same hard gil reserve used by Progression. The approval pins the character, equipment signature, item IDs, quantities, and maximum unit prices; any change requires a fresh preview. The complete shopping run holds Nexus Teleport/Navigation/Movement/UI/Inventory leases through completion or confirmed Stop. VieriAutoDuty still supplies temporary catalog discovery, vendor-window, purchase, verified-equip, gearset, and displaced-equipment mechanics while those lower-level operations migrate.
