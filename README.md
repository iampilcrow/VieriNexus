...
Progression now owns a durable gear-and-duty execution lifecycle. For the current job, it persists a target level, allowed job-quest/Hunting Log/side-quest/duty methods, a hard minimum-gil reserve, bounded task history, completion checkpoints, Last Run, Stop, and plan revision in character-scoped atomic storage. Before selecting a duty, Nexus runs its own exact gear-readiness transaction and verifies live gil, item level, and a positive completion sequence. Nexus then asks the selected AutoDuty implementation to run exactly one eligible duty, verifies the matching game duty-completion event and return to the world, and either satisfies, pauses, blocks, or schedules one fresh task. AutoDuty is used only for its bounded duty run in this flow; quest execution and general maintenance execution are still being migrated.
...
The VieriAutoDuty operations importer now preserves every profile, character assignment, retired-item transfer, compact-overlay preference, and custom gear/repair/extraction/coffer/desynthesis/turn-in/selling/registration/in-duty-maintenance policy as one verified staged snapshot. It creates the same timestamped source backup, atomic Nexus target, SHA-256 receipt, reload verification, and guarded rollback without changing or activating VieriAutoDuty. Nexus also owns a compact optional Goto/Gear/Inventory/Extras overlay containing only currently working Nexus controls; it does not fill menus with nonfunctional maintenance placeholders or fall back to stock AutoDuty's UI.
