...
Progression now owns a durable gear, quest, and duty execution lifecycle. For the current job, it persists a target level, allowed Class/Job/Role-quest, Hunting Log, general-side-quest, and duty methods, a hard minimum-gil reserve, bounded task history, completion checkpoints, Stop-after, Stop, and plan revision in character-scoped atomic storage. Before selecting work, Nexus runs its own exact gear-readiness transaction and verifies live gil, item level, and a positive completion sequence. Nexus discovers applicable Class/Job/Role quests and ordinary non-repeatable side quests from current game data, excludes quest families owned by other policies, filters them for the current job, and asks stock Questionable or the temporary VieriCodex compatibility provider to run one exact quest. The same pinned quest ID must complete and the provider must return control before Nexus replans. If Questionable has no route for a candidate, Nexus session-blacklists that ID and immediately tries the next quest or duty, with a bounded retry ceiling. When no eligible quest is available, Nexus can ask AutoDuty to run exactly one eligible duty and applies its existing game-event and safe-return verification. Hunting Log remains the next distinct Progression boundary because it needs Nexus-owned target selection and combat coordination rather than ordinary quest IPC.
