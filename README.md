...
Gear & Inventory owns both manual and automatic upgrade planning. Nexus reads the live equipped/owned items and each supported ordinary gil vendor's real game-data catalog itself, chooses the correct vendor band and job family, scores the current job's primary stat, chooses the winning item per slot, protects active EXP items, rejects off-hand purchases behind a two-handed weapon, and displays the exact replacements it selected. The user approves selected slots under the same hard gil reserve used by Progression; automatic Progression uses that same Nexus catalog, policy, and exact approval rather than VieriAutoDuty's planner. The approval pins the character, equipment signature, item IDs, quantities, maximum prices, and protected gil floor. VieriAutoDuty is temporarily retained only for physical shop-window, purchase, verified-equip, gearset, and displaced-equipment actions while those last mechanics migrate.
